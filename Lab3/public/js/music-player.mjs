...
export default {
    ...
    getRecentSongs: async (userId = null) => {
        try {
    ...
    updateFavourites: (userId, songId) => {
        ...
        getDoc(favouritesRef).then((favouritesSnapshot) => {
            ...
            setDoc(favouritesRef, {
                TrackList: favourites,
                Type: "system"
            }, {merge: true}).then(() => {
                console.log(favourites.includes(songId) ? "Song was successfully added to the favourites!"
                    : "Song was successfully removed from the favourites!")
            }).catch((error) => {
